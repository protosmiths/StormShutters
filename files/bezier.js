...
//	console.log("bez construct coords :", coords);
    ...
    this._linear = !aligned.some((p) => abs(p.y) > 0.01);
  ...
  update() {
	// for(let iIdx = 0; iIdx < this.points.length; iIdx++)
	// {
		// this.points[iIdx].x = Math.round(8.0*this.points[iIdx].x)/8.0;
		// this.points[iIdx].y = Math.round(8.0*this.points[iIdx].y)/8.0;
	// }		
  ...
  overlaps(curve) {
	...
	// console.log('curve.bbox', curve.bbox());
    const lbbox = this.bbox(),
      tbbox = curve.bbox();
	  // console.log(utils.bboxoverlap(lbbox, tbbox));
