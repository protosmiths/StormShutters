    ...
    constructor(source)
    {
        ...
        } else if (Array.isArray(source))
        {
            ...
            } else if (source[0] instanceof LoopArea)
            {
                this.bezier = [];
                this.loops = [];
                for (let loop of source)
                {
                    let newLoop = new LoopArea(loop);
                    this.loops.push(newLoop);
                    for (let bezier of newLoop.poly.curves)
                    {
                        this.beziers.push(bezier);
                    }
                }
    ...
    fromBeziers(bezierArray)
    {
        ...
        for (let bezier of bezierArray)
        {
            if (bezier instanceof Bezier)
            {
                ...
                let dist = utils.dist(bezier.get(1), loop[0].get(0));
                //console.log('dist', dist);
                if (dist < 0.01)
                {
                    //loop.push(bezier);
                    //console.log('Loop found', dist, loop);
    ...
    findIntersections(otherPath, tol=0.01)
    {
        ...
        for (let i = 0; i < this.loops.length; i++)
        {
            ...
            while (thisIntersection.path1.exit_code == 0)
            {
                ...
                let contains = otherPath.loops[otherLoop].poly.contains(midPoint);
        ...
        for (let i = 0; i < otherPath.loops.length; i++)
        {
            ...
            while (thisIntersection.path2.exit_code == 0)
            {
                ...
                let contains = this.loops[thisLoop].poly.contains(midPoint);
