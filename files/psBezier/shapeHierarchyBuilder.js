...
class bzTree
{
    ...
    */
    addNode(shape, object, tol = 0.01)
    {
        ...
        //This loop is effectively equivalent to a reentrant function.  We are walking up the branches of the tree.  We can
        //restart the loop by changing currentNode and setting i to -1. It is handy to visualize this as a reentrant function
        //that works on the tree walking up the selected branch. The fact that the branches are mutually exclusive and that
        //by design a shape can only be inside of one branch makes this work.  It is possible for a shape to be inside of
        //multiple branches, but there is no mechanism to handle this.  The first branch that is inside of the shape or the
        //shape is inside of the branch will be the branch that is taken.  The other branches will be ignored.
        for (let i = 0; i < currentNode.children.length; i++)
        {
            let branch = currentNode.children[i];
            ...
            if (relation.inside)
            {
                ...
                currentNode.children.splice(i, 1, newNode);
                //I think we are done with this node. We have found its place in the tree.  We can break out of the loop.
                return;
