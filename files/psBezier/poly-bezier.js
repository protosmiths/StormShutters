...
class PolyBezier
{
    ...
    get(global_t)
    {
        ...
        let bezierInfo = this.getBezierIndex(global_t);
        //let bezierIndex = Math.floor(global_t);
        //let local_t = global_t - bezierIndex;
        //console.log('get', global_t, bezierInfo);
        return this.curves[bezierInfo.bezierIndex].get(bezierInfo.local_t);
    }
    ...
    */
    split(global_t1, global_t2)
    {
        ...
        if (global_t2 < global_t1)
        {
            ...
            path.push(...this.split(global_t1, this.curves.length));
        ...
        //console.log('bezierInfo2', global_t2, bezierInfo2);
        if(bezierInfo2.local_t != 0) path.push(this.curves[bezierInfo2.bezierIndex].split(0, bezierInfo2.local_t));
    ...
    */
    getBezierIndex(global_t)
    {
        ...
        if (bezierIndex >= this.curves.length)
        {
            ...
            local_t = 1;
            //console.log('getBezierIndex', global_t, bezierIndex, local_t);
        }
        //console.log('getBezierIndex', global_t, bezierIndex, local_t);
    ...
    */
    ...
    /*
    * The intersection function below is a mutli-step process.  The first step is to find all the
    * intersections between the two shapes.  The second step is to sort the intersections along the
    * two paths.  The third step is to determine the transitions at the intersections.  The fourth
    * step is to determine the paths that are inside the shape.  The fifth step is to determine the
    * paths that are outside the shape.  The sixth step is to determine the paths that are on the
    * shape.  The seventh step is to determine the paths that are coincidental with the shape. 
    * 
    * There are cases where the first step is all that ia needed. We are going to split that step
    * into a separate function.  The function below will be the function that gets called by the
    * intersection function.  The function below will call the first step function and then call
    */
    rawIntersections(poly, tol)
    {
        let rawIntersections = [];
        // let limit = 4 * tol;
        for (let iIdx = 0; iIdx < this.curves.length; iIdx++)
    ...
    intersections(poly, tol)
    {
        ...
        //implemented
        let rawIntersections = this.rawIntersections(poly, tol);
        if (rawIntersections.length == 0)
            return []; //No intersections return empty array
    ...
    */
    shapeRelation(shape, tol = 0.01)
    {
        ...
        //console.log('shapeRelation', this, shape);
        //This is case where we do need the full intersection processing
        let intersects = this.rawIntersections(shape, tol)
