...
class LoopArea
{
     ...
     */
    constructor(start_t, end_t, poly, direction, firstIntersection, intersected = false)
    {
        //Clone constructor
        if (start_t instanceof LoopArea)
        {
            let loop = start_t;
            this.start_t = loop.start_t;
            this.end_t = loop.end_t;
    ...
    bezierLocalToGlobal(otherLoop, tol = 0.01)
    {
        ...
        for (let i = 0; i < this.poly.curves.length; i++)
        {
            for (let j = 0; j < otherLoop.poly.curves.length; j++)
            {
                //We need an interim object to hold the raw intersections with converted global t values
                let localIntersections = this.poly.curves[i].intersects(otherLoop.poly.curves[j], tol);
    ...
    */
    ...
    findIntersections(otherLoop, tol = 0.01)
    {
        ...
        {
            let midPoint = this.getMidPoint(this.intersections[0].path1.end_t, this.intersections[0].path1.start_t);
            ...
            midPoint = otherLoop.getMidPoint(this.intersections[0].path2.exit_t, this.intersections[0].path2.entry_t);
